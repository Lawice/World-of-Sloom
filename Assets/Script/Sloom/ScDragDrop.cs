...
public class ScDragDrop : MonoBehaviour {
    ...
    public void DragSloom() {
        ...
        SelectedSloom.GetComponent<ScSloom>().SloomState = SLOOMSTATE.Moving;
        ...
        if (Physics2D.OverlapCircle(targetPos, rayRayon, _groundLayer)){
            ...
            if (hitY.collider != null) {
                SelectedSloom.transform.position = Vector2.MoveTowards(sloomPos, new Vector2(targetPos.x, hitY.point.y+ rayRayon), 15f);
            }
        }
        else {
            SelectedSloom.transform.position = Vector2.MoveTowards(sloomPos, targetPos, 15f);
        }
    }
}

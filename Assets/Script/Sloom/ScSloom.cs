...
public class ScSloom : MonoBehaviour{
    ...
    public void Drop() {
        Collider2D[] sloomColliders = Physics2D.OverlapCircleAll(transform.position, _detectionRadius, _SloomLayer);
        
        if(sloomColliders.Length-1 >= 2) {
            Debug.Log(sloomColliders.Length);
            
            foreach (Collider2D sloomCollider in  sloomColliders) {
            ...
            if (NeighborSlooms.Count != 0  && NeighborSlooms.Count >=2) {
                SloomState = SLOOMSTATE.Placed;
            }
        }
        else {
            SloomState = SLOOMSTATE.Movable;
        }
    }
    ...
    void AddLine(GameObject gameObject1, GameObject gameObject2, bool isTransparent = false) {
        if(!IsSloomInList(gameObject2.transform)) {
            ...
            ScLine newLineComp = newLineObject.GetComponent<ScLine>();
            newLineComp.OnInstantiate();
            
            newLineComp.SetTransparancy(isTransparent);
            newLineComp.SetLineType(SloomState);
            ...
            line.GetComponent<ScLine>().SetTransparancy(isTransparent);
        }
    }
